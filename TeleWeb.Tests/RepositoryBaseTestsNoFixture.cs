...
namespace TeleWeb.Tests
{
    ...
    public class RepositoryBaseTestsNoFixture
    {
        ...
        public RepositoryBaseTestsNoFixture()
        {
            _dbFixture = new DbFixture2();
        }
        ...
        public async Task CreateAsync_Should_Add_Entity_To_DbContext()
        {
            ...
            using (var dbContext = _dbFixture.GetDbContext())
            {
                ...
                Assert.Contains(entity,
                    dbContext.Set<Channel>().Local); // Verify that the entity is added to the in-memory database
                var entity2 = new Channel { Name = "Something" };
                Assert.DoesNotContain(entity2,
                    dbContext.Set<Channel>().Local); // Verify that the entity is not added to the in-memory database
            }
        }
        ...
        public async Task UpdateAsync_Should_Update_Entity_In_DbContext()
        {
            ...
            using (var dbContext = _dbFixture.GetDbContext())
            {
                ...
                // Assert
                Assert.Equal("Test",
                    newEntity!.Name); // Verify that the entity is marked as Modified in the in-memory database
            }
        }
